 ...
 * front-ends of data warehousing, decision support, OLAP, and business intelligence systems. <P> <b>NOTE:</b> you must
 * load the Analytics {@link com.smartgwt.client.docs.LoadingOptionalModules Optional Modules} before you can use CubeGrid.
 * <P> <B>Multi-Dimensional Data Terminology</B> <P> The CubeGrid refers to the dimensions of a data cube as facets, to the
 * possible values in each facet as facet values, and to the values within the data cube as data values or cell values.
 * Equivalent terms that are commonly used in data warehousing or business intelligence systems include:<br> <b>facet:</b>
 * dimension, attribute, feature<br> <b>facet value:</b> dimension member, attribute value, feature value<br> <b>cell
 * value:</b> data value, metric value, measure <P> <B>Visual Structure</B> <P> Like the ListGrid and TreeGrid components,
 * the CubeGrid displays data values in a tabular "body" with adjacent "headers".  While the ListGrid and TreeGrid display
 * rows of records with field values, the CubeGrid displays a body of individual cell values, each associated with a
 * combination of facet values. The facet values for a cell are displayed in the column headers above the cell and row
 * headers to the left of the cell. CubeGrids can display an arbitrary number of facets, by stacking multiple levels of row
 * and/or column headers. <P> Except for the innermost column facet, each facet in a CubeGrid has a facet label adjacent to
 * its row or column headers. The facet labels serve two main purposes: they display the titles of the facets, and they
 * provide drag-and-drop reordering or pivoting of facets within the CubeGrid. The row facet labels also provide
 * interactive selection, resizing, and other operations on the columns of row facet values. <P> The innermost column
 * headers provide special behaviors and controls for manipulating the columns of data in a CubeGrid. End users may select,
 * resize, reorder, minimize, maximize, or auto-fit the columns of data via mouse interactions with these headers.
 * Customizable indicators and controls may be included at the top of each innermost column header. <P> If a CubeGrid is
 * not large enough to display all of its cell values, horizontal and/or vertical scrollbars will appear below and to the
 * right of the body. The body of the CubeGrid may be scrolled on either axis. The headers are "frozen" from scrolling on
 * one axis - row headers only scroll vertically, while column headers only scroll horizontally - so the facet values for
 * the visible cells are always displayed. <P> <B>Data Loading</B> <P> Data can be provided to the Cube via  data as an
 * Array of {@link com.smartgwt.client.widgets.cube.CellRecord CellRecords}, each representing the data for one cell. <P>
 * For large datasets, {@link com.smartgwt.client.widgets.cube.CubeGrid#getDataSource provide a DataSource} with one field
 * per facetId, and the CubeGrid will load data on demand to fill the visible area, including lazily loading data for
